...
github.com/diskfs/go-diskfs is a [go](https://golang.org) library for performing manipulation of disks, disk images and filesystems natively in go.

You can do nearly everything that github.com/diskfs/go-diskfs provides using shell tools like gdisk/fdisk/mkfs.vfat/mtools/sgdisk/sfdisk/dd. However, these have the following limitations:
...
github.com/diskfs/go-diskfs performs all modifications _natively_ in go, without mounting any disks.

## Usage
Note: detailed go documentation is available at [godoc.org](https://godoc.org/go-dparm/github.com/diskfs/go-diskfs).

### Concepts
`github.com/diskfs/go-diskfs` has a few basic concepts:
...
Note that `OpenFile()` is intended to match [os.OpenFile](https://golang.org/pkg/os/#OpenFile) and returns a `gogithub.com/diskfs/go-diskfs.File` that closely matches [os.File](https://golang.org/pkg/os/#File)
...
`gogithub.com/diskfs/go-diskfs` recognizes read-only filesystems and limits working with them to the following:
...
```go
import github.com/diskfs/go-diskfs "go-dparm/github.com/diskfs/go-diskfs"
...
disk := github.com/diskfs/go-diskfs.Create(diskImg, diskSize, github.com/diskfs/go-diskfs.Raw, github.com/diskfs/go-diskfs.SectorSizeDefault)
...
fs, err := disk.CreateFilesystem(0, github.com/diskfs/go-diskfs.TypeFat32)
...
* Integration: these test the exported functions and their ability to create or manipulate correct files. They are validated by running a [docker](https://docker.com) container with the right utilities to validate the output. These are run by running `TEST_IMAGE=github.com/diskfs/go-diskfs/gogithub.com/diskfs/go-diskfs go test ./...` or just `make test`. The value of `TEST_IMAGE` will be the image to use to run tests.
